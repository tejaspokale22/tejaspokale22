```html
<div align="center">

<h2>Software Engineer | Generative AI Explorer | Cloud & DevOps Enthusiast | Open Source Contributor</h2>

</div>
```

</br>
</br>

![B.Tech](https://img.shields.io/badge/B.Tech-Information%20Technology-black?style=for-the-badge)
![PCCOE](https://img.shields.io/badge/PCCOE-Pune-black?style=for-the-badge&logo=google-scholar&logoColor=white)
![Open%20Source](https://img.shields.io/badge/Open%20Source-Contributor-black?style=for-the-badge&logo=github&logoColor=white)

![Location](https://img.shields.io/badge/Location-Pune%2C%20India-black?style=flat-square&logo=googlemaps&logoColor=white)
![](https://komarev.com/ghpvc/?username=tejaspokale22&style=flat-square&color=000000)
![](https://img.shields.io/github/stars/tejaspokale22?style=flat-square&color=000000)

</div> 

---

# About Me

I am a Software Engineer with experience building scalable SaaS platforms, AI-powered products, blockchain applications, and enterprise-grade web systems. Passionate about solving real-world problems through technology, combining software engineering principles with AI, cloud computing, and modern development practices.

I have done my B.Tech in Information Technology at PCCOE, Pune while actively contributing to open source initiatives and developing innovative products across Web, AI, and Cloud ecosystems. Recognized as a Hacktoberfest Super Contributor and experienced in delivering production-ready solutions with strong focus on performance, security, scalability, and user experience.

### Open To

* Software Engineering Roles
* Full Stack Development
* Generative AI Roles
* Cloud & DevOps Engineering
* Open Source Contributions
* Product Engineering Opportunities

---

# Tech Stack

## Programming Languages

<p align="center">
<img src="https://skillicons.dev/icons?i=c,cpp,java,python,js,ts,sql" />
</p>

## Frontend

<p align="center">
<img src="https://skillicons.dev/icons?i=react,nextjs,html,css,tailwind,vite" />
</p>

## Backend & Databases

<p align="center">
<img src="https://skillicons.dev/icons?i=nodejs,express,spring,mongodb,mysql,postgres,redis,firebase,supabase" />
</p>

## Cloud, DevOps & Tooling

<p align="center">
<img src="https://skillicons.dev/icons?i=aws,gcp,docker,jenkins,githubactions,git,github,figma,vscode,idea" />
</p>

---

# AI / ML Expertise

| Domain                 | Proficiency  | Details                                           |
| ---------------------- | ------------ | ------------------------------------------------- |
| Generative AI          | Advanced     | LLM Applications, Prompt Engineering              |
| RAG Systems            | Advanced     | Retrieval-Augmented Generation, Knowledge Systems |
| AI Agents              | Advanced     | Agentic Workflows, Automation Systems             |
| NLP                    | Advanced     | Embeddings, Semantic Search, Text Processing      |
| Vector Databases       | Intermediate | Similarity Search & Retrieval                     |
| LLM Integrations       | Advanced     | Gemini, Hugging Face, LLaMA                       |
| AI Product Engineering | Advanced     | Production AI Features & Deployments              |

---

# Featured Projects

<details>
<summary><b>CipherDocs — Blockchain Certificate Verification Platform</b></summary>

### Overview

A Polygon blockchain-based decentralized certificate issuance and verification platform with AI-powered validation and immutable verification records.

| Category    | Details                                             |
| ----------- | --------------------------------------------------- |
| Stack       | Next.js, Polygon, Solidity, MetaMask, IPFS, Node.js |
| Scale       | Multi-user verification system                      |
| Performance | Instant QR verification                             |
| Security    | Blockchain immutability + Web3 authentication       |
| Impact      | Fraud-resistant certificate verification            |

#### Engineering Highlights

* Smart contract-based certificate storage
* IPFS distributed encrypted storage
* MetaMask authentication
* QR-based instant verification
* AI-powered validation engine
* Low gas optimization strategy

</details>

<details>
<summary><b>QuickGigs — AI Powered Freelancing Platform</b></summary>

### Overview

Built a freelancing marketplace enabling students to monetize skills through real-world projects while leveraging AI-powered freelancer recommendations.

| Category    | Details                                                      |
| ----------- | ------------------------------------------------------------ |
| Stack       | TypeScript, Next.js, Firebase, Appwrite, Gemini, HuggingFace |
| Scale       | Multi-user marketplace                                       |
| Performance | Real-time updates                                            |
| Security    | Secure payouts & authentication                              |
| Impact      | Student gig economy platform                                 |
| Repository  | Private / Available Upon Request                             |

#### Engineering Highlights

* Real-time gig management
* AI recommendation engine
* Embeddings-powered matching
* Messaging infrastructure
* QR payout workflows
* Scalable cloud architecture

</details>

<details>
<summary><b>Greenify — AI Waste Management Platform</b></summary>

### Overview

AI-powered waste reporting and management platform using image analysis, geo-tagging, verification workflows, and environmental impact tracking.

| Category    | Details                                     |
| ----------- | ------------------------------------------- |
| Stack       | Next.js, PostgreSQL, Drizzle ORM, Gemini AI |
| Scale       | Community reporting platform                |
| Performance | 90% verification accuracy                   |
| Security    | Secure reporting workflows                  |
| Impact      | Sustainable civic engagement                |
| Repository  | Private / Available Upon Request            |

#### Engineering Highlights

* Image-based reporting
* AI waste classification
* Environmental impact analysis
* Verification workflows
* Community leaderboards
* Geo-tagged intelligence

</details>

<details>
<summary><b>Fillio — Autofill Google forms Extension</b></summary>

### Overview

AI-powered browser extension enabling one-click smart form completion through intelligent field mapping and LLM-powered understanding.

| Category    | Details                                      |
| ----------- | -------------------------------------------- |
| Stack       | JavaScript, React, Vite, Tailwind CSS, LLaMA |
| Scale       | Browser extension                            |
| Performance | Instant autofill                             |
| Security    | Local processing workflows                   |
| Impact      | Productivity enhancement                     |

#### Engineering Highlights

* Intelligent field detection
* AI-based contextual mapping
* Browser automation
* User profile management
* Productivity optimization

</details>

---

# Experience

## Full Stack Developer Intern — Unihox Technologies

**June 2025 – August 2025**

Worked on AI-powered wellness SaaS products and e-commerce solutions while contributing across backend APIs, frontend systems, performance optimization, and AI integrations.

### Scope of Work

* Developed APIs for wellness analytics
* Built AI-powered workflow automation
* Enhanced SEO and website performance
* Implemented scalable backend services
* Collaborated on production deployments

**Skills**

`Next.js` `TypeScript` `Node.js` `Redis` `Docker` `CI/CD` `AI`

---

## Open Source Contributor — Hacktoberfest 2025

**October 2025**

Recognized as a Super Contributor with 10+ quality pull requests merged and ranked among the top 10k global contributors.

### Scope of Work

* Open source contributions
* Feature implementation
* Fix Bugs and issues
* Code quality improvements

**Skills**

`React.js` `Node.js` `SQL Server` `Python`

---

## Web Developer Intern — PCET Pune

**September 2024 – January 2025**

Developed academic SaaS systems automating assessment workflows, accreditation tracking, and institutional analytics.

### Scope of Work

* SaaS platform development
* Academic workflow automation
* CO-PO mapping systems
* Analytics dashboards
* Data processing pipelines

**Skills**

`React.js` `MongoDB` `Express.js` `Node.js`

---

# Achievements

<div align="center">

| Recognition             | Details                              |
| ----------------------- | ------------------------------------ |
| Open Source             | Hacktoberfest Super Contributor      |
| Competitive Programming | 600+ DSA Problems Solved             |
| SQL Expertise           | 150+ SQL Problems Solved             |
| Coding Platforms        | 1600+ Contest Rating                 |
| GitHub                  | 700+ Contributions                   |
| Leadership              | DevOps Lead - GDGC PCCOE             |
| Leadership              | Web Development Lead - Rocketry Club |

</div>

---

# Coding Profiles

<div align="center">

<a href="https://leetcode.com/tejaspokale22">
<img src="https://img.shields.io/badge/LeetCode-600%2B%20Problems-FFA116?style=for-the-badge&logo=leetcode&logoColor=white"/>
</a>

</div>

---

# GitHub Analytics

<div align="center">

<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=tejaspokale22&theme=tokyonight" />

<br><br>

<img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=tejaspokale22&theme=tokyonight" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=tejaspokale22&theme=tokyonight" />

<br><br>

<img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=tejaspokale22&theme=tokyonight" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=tejaspokale22&theme=tokyonight&utcOffset=5.5" />

</div>

---

# GitHub Trophies

<div align="center">

<img src="https://github-trophies.vercel.app/?username=tejaspokale22&theme=tokyonight&no-frame=true&margin-w=15&margin-h=15" />

</div>

---

# Connect

<div align="center">

<a href="mailto:tejaspokale22@gmail.com">
<img src="https://img.shields.io/badge/Gmail-Contact%20Me-111111?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://linkedin.com/in/tejaspokale22">
<img src="https://img.shields.io/badge/LinkedIn-Let's%20Connect-111111?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://github.com/tejaspokale22">
<img src="https://img.shields.io/badge/GitHub-Follow-111111?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="https://tejaspokale.tech">
<img src="https://img.shields.io/badge/Portfolio-Visit%20Website-111111?style=for-the-badge&logo=google-chrome&logoColor=white"/>
</a>

</div>

---

<div align="center">

*"Engineering scalable products, leveraging AI intelligently, and creating meaningful impact through technology."*

</div>
